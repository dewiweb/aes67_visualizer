...
- **Key learnings**:
  ...
  - SAP announcement format: header + `application/sdp\0` + SDP body
  - Stream timeout: 5 minutes for non-refreshed SAP sessions
  - `sdp-transform` library for SDP parsing
  - RTP audio playback via `audify` (RtAudio Node.js bindings)
  - Jitter buffer implementation for RTP reassembly
...
- **License**: Proprietary (architecture reference only)
- **Language**: JavaScript / Electron
- **Key learnings**:
  - `SapDiscovery` class as EventEmitter pattern
  - Monitoring Wall UI concept (drag-and-drop stream slots)
  - i18n multi-language architecture
  - Level polling via UDP JSON on port 8999
...
- **Language**: Python
- **Key learnings**:
  - Dante ARC protocol binary format: `start_code(0x27FF) + total_length + seqnum + opcode + result_code + content`
  - ARC UDP port: 4440
  - Key opcodes: `0x1000` (channel count), `0x1002` (device name), `0x1003` (device info),
    `0x2000` (TX channel list), `0x2010` (TX friendly names), `0x3000` (RX list + subscriptions),
    `0x3010` (set subscriptions / routing), `0x2201` (create TX flow), `0x3200` (query RX flows)
  - Result codes: `0x0001` = OK, `0x8112` = OK with more pages, `0x0022` = error
  - Pagination via result code `0x8112` (more data available)
...
- **Key learnings**:
  ...
  - Pagination mechanism using result code `0x8112`
  - Flow control via port 4455 (DBCP): `start_code=0x1102`
  - mDNS service types: `_netaudio-chan._udp` (per-TX-channel), `_netaudio-bund._udp` (bundles)
  - mDNS service types full list: `_netaudio-arc._udp`, `_netaudio-cmc._udp`, `_netaudio-dbc._udp`

### soundondigital/ravennakit
- **URL**: https://github.com/soundondigital/ravennakit
- **License**: AGPLv3 (protocol/spec reference only — no code included)
- **Language**: C++
- **Key learnings**:
  - RAVENNA RTSP URL paths: `/by-name/<stream>`, `/by-id/<n>`
  - RTSP port: 554 (standard), RTSP OPTIONS → DESCRIBE sequence
  - RTP default port: 5004 for RAVENNA streams
  - PTPv2 profiles for AES67/RAVENNA
  - mDNS DNS-SD service types: `_ravenna._tcp`, `_ravenna-session._tcp`, `_aes67._udp`
...
- **Language**: C++
- **Key learnings**:
  - Confirms RTP port 5004 and RTSP port 554 for RAVENNA
  - PTP domain parsing from SDP `a=ts-refclk:ptp=...` attribute
  - PTP clock-domain from SDP `a=clock-domain` (RAVENNA extension)
  - PTP hybrid mode: multicast + unicast negotiation
  - AM824 codec (AES3 / SMPTE ST2110-31) alongside L16/L24
  - Livewire (Axia) device discovery alongside SAP
  - `a=mediaclk:direct=...` SDP attribute for media clock

### bondagit/aes67-linux-daemon
- **URL**: https://github.com/bondagit/aes67-linux-daemon
- **License**: LGPL-2.1 (protocol reference only — no code included)
- **Language**: C++
- **Key learnings**:
  - AES67 reference Linux implementation
  - PTP grandmaster selection and BMCA algorithm
  - SAP announcement interval and session management
  - SDP `a=ts-refclk` attribute for PTP clock reference
